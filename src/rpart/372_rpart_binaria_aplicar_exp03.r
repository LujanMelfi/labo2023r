...
PARAM$rpart$cp <- -0.448229852977894

PARAM$rpart$minsplit <- 1163.12615244169

PARAM$rpart$minbucket <- 574.362337084919
...
for (corte in c(8500, 9000, 9500, 10000, 10500, 11000, 11500)) {
  ...
  fwrite(dfinal[, list(numero_de_cliente, Predicted)],
         file = paste0(PARAM$experimento, "_003_", corte, ".csv"),
         sep = ","
  )
}