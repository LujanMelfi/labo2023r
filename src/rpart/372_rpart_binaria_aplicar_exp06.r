...
PARAM$rpart$cp <- -0.295429484627837

PARAM$rpart$minsplit <- 1205.80382144268

PARAM$rpart$minbucket <- 602.243634966469
...
for (corte in c(8500, 9000, 9500, 10000, 10500, 11000, 11500)) {
  ...
  fwrite(dfinal[, list(numero_de_cliente, Predicted)],
         file = paste0(PARAM$experimento, "_006_", corte, ".csv"),
         sep = ","
  )
}