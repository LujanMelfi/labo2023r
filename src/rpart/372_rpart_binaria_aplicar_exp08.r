...
PARAM$rpart$cp <- -0.941569009083239

PARAM$rpart$minsplit <- 1219.21165056675

PARAM$rpart$minbucket <- 603.718500170722
...
for (corte in c(8500, 9000, 9500, 10000, 10500, 11000, 11500)) {
  ...
  fwrite(dfinal[, list(numero_de_cliente, Predicted)],
         file = paste0(PARAM$experimento, "_008_", corte, ".csv"),
         sep = ","
  )
}