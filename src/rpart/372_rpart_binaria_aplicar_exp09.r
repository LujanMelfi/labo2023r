...
PARAM$rpart$cp <- -0.291809100512415

PARAM$rpart$minsplit <- 1456.77825256504

PARAM$rpart$minbucket <- 721.028763727303
...
for (corte in c(8500, 9000, 9500, 10000, 10500, 11000, 11500)) {
  ...
  fwrite(dfinal[, list(numero_de_cliente, Predicted)],
         file = paste0(PARAM$experimento, "_009_", corte, ".csv"),
         sep = ","
  )
}