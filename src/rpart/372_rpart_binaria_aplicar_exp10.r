...
PARAM$rpart$cp <- -0.275563426116950

PARAM$rpart$minsplit <- 2023.34535798654

PARAM$rpart$minbucket <- 749.056983783102
...
for (corte in c(8500, 9000, 9500, 10000, 10500, 11000, 11500)) {
  ...
  fwrite(dfinal[, list(numero_de_cliente, Predicted)],
         file = paste0(PARAM$experimento, "_010_", corte, ".csv"),
         sep = ","
  )
}