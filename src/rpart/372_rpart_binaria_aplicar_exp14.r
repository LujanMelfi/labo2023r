...
PARAM$rpart$cp <- -0.275827536627379

PARAM$rpart$minsplit <- 1402.10867288207

PARAM$rpart$minbucket <- 697.390199020779
...
for (corte in c(8500, 9000, 9500, 10000, 10500, 11000, 11500)) {
  ...
  fwrite(dfinal[, list(numero_de_cliente, Predicted)],
         file = paste0(PARAM$experimento, "_014_", corte, ".csv"),
         sep = ","
  )
}