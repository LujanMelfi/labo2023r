...
PARAM$rpart$cp <- -0.456215060528541

PARAM$rpart$minsplit <- 1969.90888342895

PARAM$rpart$minbucket <- 584.948999071618
...
for (corte in c(8500, 9000, 9500, 10000, 10500, 11000, 11500)) {
  ...
  fwrite(dfinal[, list(numero_de_cliente, Predicted)],
         file = paste0(PARAM$experimento, "_015_", corte, ".csv"),
         sep = ","
  )
}