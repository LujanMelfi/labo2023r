...
PARAM$rpart$cp <- -0.492771986062063

PARAM$rpart$minsplit <- 2019.57929662541

PARAM$rpart$minbucket <- 614.959829885958
...
for (corte in c(8500, 9000, 9500, 10000, 10500, 11000, 11500)) {
  ...
  fwrite(dfinal[, list(numero_de_cliente, Predicted)],
         file = paste0(PARAM$experimento, "_017_", corte, ".csv"),
         sep = ","
  )
}