...
PARAM$rpart$cp <- -0.442536945458199

PARAM$rpart$minsplit <- 1631.07161993684

PARAM$rpart$minbucket <- 603.251644835877
...
for (corte in c(8500, 9000, 9500, 10000, 10500, 11000, 11500)) {
  ...
  fwrite(dfinal[, list(numero_de_cliente, Predicted)],
         file = paste0(PARAM$experimento, "_018_", corte, ".csv"),
         sep = ","
  )
}