...
PARAM$rpart$cp <- -0.237636895443698

PARAM$rpart$minsplit <- 1282.41176426315

PARAM$rpart$minbucket <- 636.748474506806
...
for (corte in c(8500, 9000, 9500, 10000, 10500, 11000, 11500)) {
  ...
  fwrite(dfinal[, list(numero_de_cliente, Predicted)],
         file = paste0(PARAM$experimento, "_020_", corte, ".csv"),
         sep = ","
  )
}