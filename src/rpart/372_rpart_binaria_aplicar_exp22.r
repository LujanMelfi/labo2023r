...
PARAM$rpart$cp <- -0.636328104481468

PARAM$rpart$minsplit <- 2082.15799196739

PARAM$rpart$minbucket <- 606.244717365974
...
for (corte in c(8500, 9000, 9500, 10000, 10500, 11000, 11500)) {
  ...
  fwrite(dfinal[, list(numero_de_cliente, Predicted)],
         file = paste0(PARAM$experimento, "_022_", corte, ".csv"),
         sep = ","
  )
}