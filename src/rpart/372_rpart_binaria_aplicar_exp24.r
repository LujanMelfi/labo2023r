...
PARAM$rpart$cp <- -0.619266485458244

PARAM$rpart$minsplit <- 2051.44378698765

PARAM$rpart$minbucket <- 608.537882215823
...
for (corte in c(8500, 9000, 9500, 10000, 10500, 11000, 11500)) {
  ...
  fwrite(dfinal[, list(numero_de_cliente, Predicted)],
         file = paste0(PARAM$experimento, "_024_", corte, ".csv"),
         sep = ","
  )
}