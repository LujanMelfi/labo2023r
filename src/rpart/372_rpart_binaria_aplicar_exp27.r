...
PARAM$rpart$cp <- -0.596724534059246

PARAM$rpart$minsplit <- 1999.95398403317

PARAM$rpart$minbucket <- 646.270100274350
...
for (corte in c(8500, 9000, 9500, 10000, 10500, 11000, 11500)) {
  ...
  fwrite(dfinal[, list(numero_de_cliente, Predicted)],
         file = paste0(PARAM$experimento, "_027_", corte, ".csv"),
         sep = ","
  )
}